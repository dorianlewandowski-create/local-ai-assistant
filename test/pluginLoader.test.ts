...
test('plugin loader discovers valid external plugins', () => {
  const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apex-plugins-'))
  const pluginRoot = path.join(pluginDir, 'sample')
  fs.mkdirSync(pluginRoot, { recursive: true })
  fs.writeFileSync(
    path.join(pluginRoot, 'apex-plugin.json'),
    JSON.stringify({
      id: 'sample-plugin',
...
test('plugin loader skips invalid plugin manifests safely', () => {
  const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apex-plugins-invalid-'))
