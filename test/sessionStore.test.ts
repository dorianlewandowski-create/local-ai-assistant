...
test('session store supports per-session settings', async () => {
  const task = createTask({ source: 'terminal', sourceId: 'local-console-session' })
  const settings = await sessionStore.updateSessionSettings(task, {
    verbosity: 'high',
    approvalMode: 'strict',
  })
...
test('session store persists and restores session snapshots', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apex-sessions-'))
  ...
  await store.updateSessionSettings(task, { verbosity: 'medium' })
...
test('session store prunes persisted sessions by retention limit', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'apex-sessions-prune-'))
  const filePath = path.join(tempDir, 'sessions.json')
  const store = new SessionStore(filePath, 2)

  await store.appendInteraction(createTask({ sourceId: 'one' }), 'a', 'a')
  await new Promise((resolve) => setTimeout(resolve, 5))
  await store.appendInteraction(createTask({ sourceId: 'two' }), 'b', 'b')
  await new Promise((resolve) => setTimeout(resolve, 5))
  await store.appendInteraction(createTask({ sourceId: 'three' }), 'c', 'c')
