...
import type { Message, SubAgentKind, TaskEnvelope, TaskSource } from '@apex/types'
...
export interface SessionSettings {
  ...
  /** When set, overrides heuristic sub-agent routing (see AgentFactory.choose). */
...
export class SessionStore {
  ...
  async appendInteraction(task: TaskEnvelope, prompt: string, response: string): Promise<void> {
  ...
  private evictBeforeCreate(): void {
    while (this.sessions.size >= this.maxSessions) {
      const oldest = Array.from(this.sessions.values()).sort((left, right) =>
  ...
  private persistToDisk(): Promise<void> {
    // Serialize writes so concurrent tasks can't interleave payloads.
    this.persistChain = this.persistChain
      .then(() => this.persistToDiskAtomic())
      .catch(() => this.persistToDiskAtomic())
    return this.persistChain
  }

  private async persistToDiskAtomic(): Promise<void> {
    const payload: PersistedSessionState = {
      sessions: Array.from(this.sessions.values())
        .sort((left, right) => left.updatedAt.localeCompare(right.updatedAt))
        .slice(-this.maxSessions),
      sourceHistory: Array.from(this.sourceHistory.entries()).map(([key, entries]) => [
        key,
        entries.slice(-MAX_SOURCE_HISTORY),
      ]),
    }
