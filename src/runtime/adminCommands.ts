...
function isRemoteSource(source: TaskEnvelope['source']): boolean {
  return source === 'telegram' || source === 'slack' || source === 'whatsapp' || source === 'discord'
}
    ...
    switch (command.name) {
      ...
      case 'queue': {
        ...
        const queues =
          snapshot.queues.length === 0
            ...
            : snapshot.queues
                .map((queue) => `- ${queue.key}: active=${queue.active} pending=${queue.pending}`)
                .join('\n')
      ...
      case 'sessions': {
        ...
        return [
          ...
          ...sessions.map(
            (session) => `- ${session.key} | history=${session.historyCount} | updated=${session.updatedAt}`,
          ),
        ].join('\n')
      }
      case 'memory': {
      ...
      case 'agent': {
        const mode = command.args[0]?.toLowerCase()
        if (isRemoteSource(task.source) && mode && mode !== 'status') {
          return 'Sub-agent override can only be changed from the local terminal.'
        }
        if (!mode || mode === 'status') {
          const current = dependencies.api.getSessionSubAgentKind(task)
          return [
            `Sub-agent routing: ${current ? `locked to ${current}` : 'automatic (keyword heuristics)'}`,
            'Use: /agent researcher | /agent coder | /agent system | /agent auto',
          ].join('\n')
        }
        if (mode === 'auto') {
          dependencies.api.setSessionSubAgentKind(task, undefined)
          return 'Sub-agent override cleared. Routing is automatic again.'
        }
        if (mode === 'researcher' || mode === 'coder' || mode === 'system') {
          dependencies.api.setSessionSubAgentKind(task, mode)
          return `Sub-agent locked to ${mode} for this session.`
        }
        return 'Unknown mode. Use: /agent researcher | coder | system | auto | status'
      }
      case 'approvals': {
