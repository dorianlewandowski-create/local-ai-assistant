import type { TaskEnvelope, TaskResult } from '@apex/types'
import { logger } from '../utils/logger'
import { emitDebugLog } from './debugIngest'
import { sessionStore } from './sessionStore'

/** Merge session-level sub-agent override into task metadata before orchestration. */
export function enrichTaskWithSessionMetadata(task: TaskEnvelope): TaskEnvelope {
  const sub = sessionStore.getSession(task).settings.subAgentKind
  if (!sub) {
    return task
  }
  return {
    ...task,
    metadata: { ...(task.metadata ?? {}), subAgentKind: sub },
  }
}
...
export class TaskQueue {
  ...
  enqueue(task: TaskEnvelope): Promise<TaskResult> {
    const enriched = enrichTaskWithSessionMetadata(task)
    return new Promise<TaskResult>((resolve, reject) => {
      const queueKey = this.resolveQueueKey(enriched)
      ...
      logger.debug(`Queue enqueued ${queueKey} ${enriched.source}:${enriched.id}`)
  ...
  private async executeQueuedTask(queueKey: string, queue: QueueState, queuedTask: QueuedTask) {
    ...
    emitDebugLog({
      ...
      runId: 'pre-fix',
      hypothesisId: 'H3',
      location: 'src/runtime/taskQueue.ts:executeQueuedTask',
      message: 'Queue executing task',
      correlationId,
      data: {
        queueKey,
        source: task.source,
    ...
    try {
      ...
      emitDebugLog({
        ...
        hypothesisId: 'H3',
        location: 'src/runtime/taskQueue.ts:executeQueuedTask',
        message: 'Queue finished task',
        correlationId,
        data: {
          respTrimLen: String((result as any)?.response ?? '').trim().length,
          agent: String((result as any)?.agent ?? ''),
          correlationId,
        },
        timestamp: Date.now(),
      })
      ...
      emitDebugLog({
        ...
        hypothesisId: 'H3',
        location: 'src/runtime/taskQueue.ts:executeQueuedTask',
        message: 'Queue failed task',
        correlationId,
        data: { err: String(message), correlationId },
        timestamp: Date.now(),
      })
