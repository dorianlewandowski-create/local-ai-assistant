...
export async function runOnboard(write: (line: string) => void = console.log): Promise<number> {
  ...
  ensureFile(path.join(root, 'apex.json'), path.join(root, 'apex.json.example'), write)

  write('Next steps:')
  write('1. Fill in .env secrets and tokens')
  write('2. Adjust apex.json settings if needed')
  write('3. Run: npm run doctor (once apex daemon is running: apex runtime-info for a quick snapshot)')
