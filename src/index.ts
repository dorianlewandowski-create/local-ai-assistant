import './loadEnv'
import { runApex } from './core/apexApp'
...
export function resolveCliCommand(argv: string[]) {
  ...
  if (command === 'help' || command === '--help' || command === '-h') {
    return { command: 'help' as const, argv: rest }
  }

  if (command === 'version' || command === '--version' || command === '-v') {
    return { command: 'version' as const, argv: rest }
  }

  if (command === 'info' || command === '--info') {
    return { command: 'info' as const, argv: rest }
  }
  ...
  if (command === 'runtime-info') {
    return { command: 'runtime-info' as const, argv: rest }
  }
  ...
  if (command === 'files') {
    return { command: 'files' as const, argv: rest }
  }
