...
const ReadTextFileParams = z.object({
  path: z
    .string()
    .describe(
      'Absolute or relative path to a UTF-8 text file. Supported extensions: .txt, .md, .js, .ts, .json.',
    ),
})
...
export const readTextFile: Tool<typeof ReadTextFileParams> = {
  name: 'read_text_file',
  description:
    'Read plain text-based files such as txt, md, js, ts, and json and return a string summary of the contents.',
  ...
  execute: async ({ path: filePath }) => {
    try {
      ...
      if (!TEXT_FILE_EXTENSIONS.has(extension)) {
        throw new Error(
          `Unsupported text file extension: ${extension || 'none'}. Use this tool only for .txt, .md, .js, .ts, or .json files.`,
        )
      }

      const content = await fs.readFile(filePath, 'utf-8')
      const summary = summariseExtractedText(filePath, content)
      await getVectorStore().store({
        source: filePath,
        scope: 'file',
        content,
...
const ReadPdfContentParams = z.object({
  path: z
    .string()
    .describe('Absolute or relative path to a PDF document that should be parsed for text content.'),
})

export const readPdfContent: Tool<typeof ReadPdfContentParams> = {
  name: 'read_pdf_content',
  description:
    'Extract raw text from a PDF document and return a string summary that can be used for reflection and follow-up actions.',
...
const AnalyzeImageContentParams = z.object({
  path: z
    .string()
    .describe('Absolute or relative path to an image file. Supported extensions: .jpg, .jpeg, .png.'),
  prompt: z
    .string()
    .optional()
    .describe(
      'Optional analysis goal for the vision model, such as describe the scene, extract text, or identify important objects.',
    ),
})
...
export const analyzeImageContent: Tool<typeof AnalyzeImageContentParams> = {
  ...
  description:
    'Send an image to a vision-capable Ollama model and return a string summary describing the image contents.',
  ...
  execute: async ({ path: filePath, prompt }) => {
    try {
      ...
      if (!IMAGE_FILE_EXTENSIONS.has(extension)) {
        throw new Error(
          `Unsupported image extension: ${extension || 'none'}. Use this tool only for .jpg, .jpeg, or .png files.`,
        )
      }
      ...
      const description = await ollamaVisionProvider.analyzeImage(
        ...
        filePath,
        prompt ||
          'Describe this image in a concise but useful way for an autonomous desktop agent. Mention notable objects, visible text, document type, and anything action-worthy.',
      )
      const summary = description
        ? `Image analysis for ${filePath}: ${description}`
        : `Image analysis for ${filePath}: the vision model returned an empty description.`
      await getVectorStore().store({
        source: filePath,
        scope: 'file',
        content: summary,
