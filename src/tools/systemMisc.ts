...
const GetCurrentWeatherParams = z.object({
  location: z
    ...
    .describe(
      'Required. The city name, optionally including state or country, such as "London" or "San Francisco, CA". Do not leave this empty.',
    ),
})
    ...
    try {
      const data: any = await fetchJsonWithTimeout(
        ...
        {},
        {
          ...
          timeoutMessage: `Weather request timed out after ${TOOL_HTTP_TIMEOUT_MS}ms`,
        },
      )
...
const SendSystemNotificationParams = z.object({
  message: z
    ...
    .describe('The visible notification body shown to the user. Keep it concise and action-oriented.'),
  title: z.string().optional().describe('Optional notification title. Defaults to "Apex".'),
  subtitle: z
    .string()
    .optional()
    .describe('Optional notification subtitle describing the action taken. Defaults to "Action taken".'),
})

export const sendSystemNotification: Tool<typeof SendSystemNotificationParams> = {
  name: 'send_system_notification',
  description:
    'Show a macOS system notification to inform the user about an autonomous action or important update.',
