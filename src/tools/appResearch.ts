...
export const researchAppAutomation: Tool<typeof ResearchAppAutomationParams> = {
  ...
  description:
    'Research how to automate a specific macOS application. It finds the app path and searches for AppleScript or CLI documentation.',
  ...
  execute: async ({ appName }) => {
    try {
      ...
      const { stdout: pathStdout } = await execAsync(
        `mdfind "kMDItemKind == 'Application' && kMDItemDisplayName == '${appName}'" | head -n 1`,
      )
