...
export const checkpointStart: Tool<typeof CheckpointStartParams> = {
  ...
  description:
    'Create a safety checkpoint for a list of files. Use this before performing high-risk file operations or multi-file edits.',
...
export const checkpointRollback: Tool<typeof CheckpointRollbackParams> = {
  ...
  execute: async ({ id }) => {
    try {
      ...
      return {
        success: true,
        result: `Successfully rolled back checkpoint '${id}'. Restored files: ${restored.join(', ')}`,
      }
    } catch (error: any) {
      return { success: false, error: error.message }
    }
...
export const checkpointCommit: Tool<typeof CheckpointCommitParams> = {
  ...
  description:
    'Finalize a checkpoint and delete the backups. Use this when you are certain the operation was successful.',
