...
export async function findRelevantExperience(task: string, limit = 3): Promise<ExperienceRecord[]> {
  const matches = await getVectorStore().searchSimilar(task, limit)
  return matches
    ...
    .map((match) => ({
      ...
      error:
        typeof match.metadata?.error === 'string'
          ? match.metadata.error
          : 'Previous task was slow or unreliable.',
      successPlan:
        typeof match.metadata?.successPlan === 'string' ? match.metadata.successPlan : match.content,
