...
import type { TaskSource, Tool } from '@apex/types'
...
function isChannelToolAllowed(toolName: string, source: TaskSource): boolean {
  if (source !== 'telegram' && source !== 'slack' && source !== 'whatsapp' && source !== 'discord') {
    return true
  }

  const allowlist = config.security.channelToolAllowlists[source]
  if (!allowlist || allowlist.length === 0) {
    console.warn(`[Security] Tool ${toolName} blocked: No allowlist defined for ${source}. Update your .env.`)
    return false
  }
...
export function assessToolRisk(
  ...
  let reason =
    permissionClass === 'read' ? 'Read-only tool.' : 'Tool affects local state and requires review.'
  ...
  if (protectedPath) {
    ...
    const matchedKeyword = DANGEROUS_KEYWORDS.find((keyword) =>
      new RegExp(`\\b${keyword}\\b`, 'i').test(serialized),
    )
    if (matchedKeyword) {
      permissionClass = 'destructive'
  ...
  if (
    isSandboxActive(source, sessionSettings) &&
    !config.security.remoteSandboxAllowedPermissions.includes(permissionClass)
  ) {
    return {
      allowed: false,
      requiresAuthorization: false,
