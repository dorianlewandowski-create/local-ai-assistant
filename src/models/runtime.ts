...
import { ChatCompletionRequest, ChatCompletionResponse, ChatModelProvider, EmbeddingModelProvider } from './provider';
...
export async function embedWithFallback(
  ...
  for (let index = 0; index < models.length; index += 1) {
    ...
    try {
      ...
      lastError = error;
      logger.warn(`Embedding model ${model} failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
