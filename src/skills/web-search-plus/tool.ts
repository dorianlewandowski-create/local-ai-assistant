...
const WebSearchPlusParams = z.object({
  ...
  provider: z.enum(['auto', 'serper', 'tavily', 'querit', 'exa', 'perplexity', 'you', 'searxng']).default('auto').describe('Specific provider to use or "auto" for intelligent routing.'),
  max_results: z.number().default(5).describe('Maximum number of results to return.'),
  explain_routing: z.boolean().default(false).describe('If true, explains why a specific provider was chosen.'),
});
...
export const webSearchPlus: Tool<typeof WebSearchPlusParams> = {
  ...
  description: 'Advanced web search with intelligent auto-routing between 7+ providers (Google, Research, Neural, AI Answers). Use this as your primary search tool for the most accurate and relevant results.',
  ...
  execute: async ({ query, provider, max_results, explain_routing }: WebSearchPlusArgs) => {
    try {
      ...
      return { success: false, error: `Web Search Plus failed. Ensure Python dependencies are installed and API keys are set in .env. Error: ${error.message}` };
    }
  },
};
