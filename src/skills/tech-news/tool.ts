...
export const runNewsPipeline: Tool<typeof RunPipelineParams> = {
  ...
  execute: async ({ hours }: { hours: number }) => {
    try {
      ...
      const { stdout: summary } = await execAsync(`python3 "${path.join(SKILL_DIR, 'scripts', 'summarize-merged.py')}" --input ${outputJson} --top 10`);
      ...
      return { success: false, error: `News pipeline failed. Ensure Python dependencies are installed. Error: ${error.message}` };
    }
  },
};
...
export const readNewsSummary: Tool<typeof ReadSummaryParams> = {
  ...
  execute: async () => {
    try {
      ...
      const { stdout: summary } = await execAsync(`python3 "${path.join(SKILL_DIR, 'scripts', 'summarize-merged.py')}" --input ${inputJson} --top 10`);
