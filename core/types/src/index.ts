...
export interface Tool<T extends z.ZodObject<any> = any> {
  ...
  parameters: T
  /**
   * Optional JSON schema for parameters. If present, this is preferred over
   * converting the Zod schema at runtime. Useful for worker-based plugins where
   * tools are declared via manifests.
   */
...
export type SystemEvent<TType extends string = string, TData = unknown> = {
  type: TType
  data?: TData
  atMs: number
}
