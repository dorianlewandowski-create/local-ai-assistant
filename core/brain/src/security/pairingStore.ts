...
function getPairingStoreFilePath(): string {
  return (
    readApexAliasedString(process.env, 'APEX_PAIRING_STORE_PATH') ||
    path.join(process.cwd(), 'data', 'telegram-pairings.json')
  )
}
...
function loadData(): PairingStoreData {
  try {
    ...
    return {
      authorizedTelegramUsers: Array.isArray(parsed.authorizedTelegramUsers)
        ? parsed.authorizedTelegramUsers.map(String)
        : [],
    }
  } catch {
